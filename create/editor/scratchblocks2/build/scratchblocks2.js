...
var scratchblocks2 = function ($) {
    ...
    var sb2 = {};
    var override_categories = ["motion", "looks", "sound", "pen", "variables", "list", "events", "control", "sensing", "operators", "custom", "custom-arg", "extension", "grey", "obsolete"];
    var override_flags = ["cstart", "celse", "cend", "ring"];
    var override_shapes = ["hat", "cap", "stack", "embedded", "boolean", "reporter"];
    ...
    for (var i = 0; i < english_blocks.length; i++) {
        if (english_blocks[i].length === 1) {
            ...
            var block_and_flags = english_blocks[i],
                spec = block_and_flags[0],
                flags = block_and_flags[1];
    ...
    block_info_by_id["_ of _"].hack = function (info, args) {
        ...
        info.category = $.inArray(func, strings.math) > -1 ? "operators" : "sensing"
    };
    block_info_by_id["length of _"].hack = function (info, args) {
        if (!args.length) return;
        info.category = /^\[.* v\]$/.test(args[0]) ? "list" : "operators"
    };
    block_info_by_id["stop _"].hack = function (info, args) {
        if (!args.length) return;
        var what = minify(strip_brackets(args[0]).replace(/ v$/, ""));
        info.shape = $.inArray(what, strings.osis) > -1 ? null : "cap"
    };
    ...
    function minify(text) {
        var minitext = text.replace(/[ \t.,%?:â–¶â—€â–¸â—‚]/g, "").toLowerCase();
    ...
    function parse_block(code, context, dont_strip_brackets) {
        ...
        for (var i = 0; i < strings.define.length; i++) {
            ...
            if (code.toLowerCase() === define_text || pieces[0] && pieces[0].toLowerCase().startsWith(define_text + " ")) {
        ...
        var pieces = [];
        var text_parts = info.spec.split(/([_@â–¶â—€â–¸â—‚])/);
    ...
    function parse_line(line, context) {
        ...
        var info;
        if (is_open_bracket(line.charAt(0)) && split_into_pieces(line).length === 1) {
            info = parse_block(line, context);
            if (!info.category) {
                info = {
                    blockid: "_",
                    category: "obsolete",
    ...
    function parse_scripts(code) {
        ...
        for (i = 0; i < lines.length; i++) {
            ...
            if (!info.pieces.length && info.comment !== undefined && nesting.length <= 1) {
                new_script();
            ...
            case "celse":
                ...
                var cwrap = nesting[nesting.length - 1];
                info.category = cwrap[0].category;
                cwrap.push(info);
                var cmouth = {
                    type: "cmouth",
    ...
    function render_stack_item(info) {
        ...
        case "cwrap":
            ...
            return $cwrap;
        case "cmouth":
            return render_stack(info.contents).addClass("cmouth").addClass(info.category);
        default:
    ...
    function render_comment(info) {
        var $comment = $(document.createElement("div")).addClass("comment").append($(document.createElement("div")).append(document.createTextNode(info.comment.trim() || " ")));
        if (info.shape) {
    ...
    function render_block(info) {
        ...
        if (info.shape === "color") {
            $block.css({
                "background-color": info.pieces[0]
            });
            $block.text(" ");
            return $block
        }
        var $ring;
        if (info.is_ringed) {
            $ring = $(document.createElement("div")).addClass("ring-inner").addClass(info.shape).append($block)
        }
        if (info.flag === "ring") {
            $block.addClass("ring")
        }
        ...
        for (var i = 0; i < info.pieces.length; i++) {
            ...
            if (typeof piece === "object") {
                ...
                $block.append($image)
            } else if (/[â–¶â—€â–¸â—‚]/.test(piece)) {
                $block.append($(document.createElement("span")).addClass("arrow").append(document.createTextNode(piece)))
            } else {
                if (!piece) piece = " ";
